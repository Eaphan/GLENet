...
class DataAugmentor(object):
    ...
    def noise_per_object(self, data_dict=None, config=None):
        ...
        # print("#### names and mask", data_dict['gt_names'], data_dict['gt_boxes_mask'])
        # import pdb;pdb.set_trace()
        # dict_keys(['frame_id', 'calib', 'gt_names', 'gt_boxes', 'gt_uncertaintys', 'road_plane', 'points', 'gt_boxes_mask'])
        # data_dict['gt_names'] = data_dict['gt_names'][0:1]
        # data_dict['gt_boxes'] = data_dict['gt_boxes'][0:1, :]
        # data_dict['gt_uncertaintys'] = data_dict['gt_uncertaintys'][0:1]
        # data_dict['gt_boxes_mask'] = data_dict['gt_boxes_mask'][0:1]

        num_original_point = data_dict['num_original_point']
        gt_boxes, points, point_masks, num_original_point = augmentor_utils.noise_per_object(
            data_dict['gt_boxes'],
            data_dict['points'],
            data_dict['num_original_point'],
            # valid_mask=data_dict['gt_boxes_mask'] if 'gt_boxes_mask' in data_dict else np.ones(data_dict['gt_boxes'].shape[0]),
            valid_mask=data_dict['gt_boxes_mask'],
            rotation_perturb=config["GT_ROTATION_NOISE"], # np.pi / 4,
            center_noise_std=config["GT_LOC_NOISE_STD"], # [1.0, 1.0, 0.5],
            num_try=100, 
            data_aug_with_context=config["DATA_AUG_WITH_CONTEXT"], # -1.0,
            data_aug_random_drop=config["DATA_AUG_RANDOM_DROP"], #-1.0
        )
        data_dict['num_original_point'] = num_original_point
        data_dict['gt_boxes'] = gt_boxes
        data_dict['points'] = points
        # data_dict['point_masks'] = point_masks
        return data_dict
