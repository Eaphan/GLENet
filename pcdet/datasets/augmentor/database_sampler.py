...
class DataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        if 'gt_uncertaintys' in data_dict:
            sampled_gt_uncertaintys = np.array([x['uncertainty'] for x in total_valid_sampled_dict])
            sampled_gt_uncertaintys = np.zeros([0,7]) if len(sampled_gt_uncertaintys)==0 else sampled_gt_uncertaintys
        sampled_gt_boxes_mask = np.array([True for x in total_valid_sampled_dict])

        large_sampled_gt_boxes = box_utils.enlarge_box3d(
            sampled_gt_boxes[:, 0:7], extra_width=self.sampler_cfg.REMOVE_EXTRA_WIDTH
        )
        points = box_utils.remove_points_in_boxes3d(points, large_sampled_gt_boxes)
        num_original_point = points.shape[0]
        points = np.concatenate([points, obj_points], axis=0)
        gt_names = np.concatenate([gt_names, sampled_gt_names], axis=0)
        gt_boxes = np.concatenate([gt_boxes, sampled_gt_boxes], axis=0)
        gt_boxes_mask = np.concatenate([gt_boxes_mask, sampled_gt_boxes_mask], axis=0)
        ...
        if 'gt_uncertaintys' in data_dict:
            gt_uncertaintys = np.concatenate([gt_uncertaintys, sampled_gt_uncertaintys], axis=0)
            data_dict['gt_uncertaintys'] = gt_uncertaintys
        return data_dict
    ...
    def __call__(self, data_dict):
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            data_dict['num_original_point'] = len(data_dict['points'])
